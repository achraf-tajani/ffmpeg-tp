...
(async function () {
    try {
        ...
        await exec(`"${pathToFfmpeg}" -start_number 1 -i ${outputFolder}/%d.png -vcodec ${videoEncoder} -pix_fmt yuv420p ${outputFile}`)
...
function log(elm, message) { console.log(`${message}[${elm}]`) }
function error(elm, message) { console.log(`ERROR : ${message}[${elm}]`) }
